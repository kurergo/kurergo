(function () {
  ...
  function initCalculator() {
    ...
    function calculateIncome() {
      ...
      var cityMultiplier = CITY_RATE_MULTIPLIER[state.city] || 1;
      var workIncome = state.day * state.hour * rate * cityMultiplier * WEEKS_IN_MONTH;
      return workIncome;
    }
    ...
    function renderIncome() {
      ...
      document.querySelectorAll(".js_city, #js_city").forEach(function (node) {
        node.textContent = state.city;
        node.style.visibility = "visible";
        node.style.opacity = "1";
      });
    }

    var citySelect = calculator.querySelector("#dropdown-city");
    if (citySelect) {
      var cityValue = citySelect.value || citySelect.getAttribute("placeholder");
      if (CITY_RATE_MULTIPLIER[cityValue]) state.city = cityValue;
      citySelect.value = state.city;
      citySelect.addEventListener("change", function () {
        state.city = CITY_RATE_MULTIPLIER[citySelect.value] ? citySelect.value : "Москва";
        renderIncome();
      });
    }
